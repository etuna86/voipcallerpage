jQuery(function ($) {
    ...
    $('.extra-save-btn').click(function(e){
        var save_btn_new_link  = $(this).data('save-btn-new-link');
        $('.crud-form').parent().attr('action',save_btn_new_link);
    });
    $('.base-save-btn').click(function(e){
        if($('.extra-save-btn').length > 0){
            var save_btn_base_link  = $(this).data('save-btn-base-link');
            $('.crud-form').parent().attr('action',save_btn_base_link);
        }
    });
                    ...
                    $('#crudForm').ajaxSubmit({
                        ...
                        success: function(result){
                            ...
                            if(data.success)
                            {
                                ...
                                if(typeof (data.success_list_url) != 'undefined' ){
                                    setTimeout(function(){
                                ...
                                clearForm();
                                form_success_message(data.msg);
                            }
                            else
                            {

                                form_error_message( data.msg );
                            }
                        },
                        error: function(){
...
function clearForm()
{
    ...
    //empty select from to
    $('select[name="to[]"]').find('option').attr('selected','selected');
    setTimeout(function(){
        $('#left_All_1').click();
    },50);
    setTimeout(function(){
        $('select[name="from[]"]').not('.dont-empty-this').find('option').remove();
    },150);
